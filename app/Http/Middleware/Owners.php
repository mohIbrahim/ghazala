...
class Owners
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if($request->route()->getName() == 'owners.index'      && in_array('view_owners', $permissions)){
        ...
        if($request->route()->getName() == 'owners.create'     && in_array('create_owners', $permissions)){

            $response = $next($request);
        }else

        if($request->route()->getName() == 'owners.store'      && in_array('create_owners', $permissions)){

            $response = $next($request);
        }else

        if($request->route()->getName() == 'owners.edit'       && in_array('update_owners', $permissions)){
            
            $response = $next($request);
        }else

        if($request->route()->getName() == 'owners.update'     && in_array('update_owners', $permissions)){
